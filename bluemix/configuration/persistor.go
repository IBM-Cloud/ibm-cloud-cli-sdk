...
func (dp *DiskPersistor) lockedRead(data DataInterface) error {
	lockCtx, cancelLockCtx := context.WithTimeout(dp.parentContext, 100*time.Second) /* allotting a 30-second timeout means there can be a maximum of 28 failed retrials (each up to 500 ms, as
	...
	defer cancelLockCtx()
	_, lockErr := dp.fileLock.TryLockContext(lockCtx, 678*time.Millisecond) /* provide a file lock just while dp.read is called, because it calls an unmarshaling function
